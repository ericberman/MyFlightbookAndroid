...
public class CategoryClass extends SoapableObject implements KvmSerializable, Serializable {
	...
	public enum CatClassID {none, ASEL, AMEL, ASES, AMES, Glider, Helicopter, Gyroplane, PoweredLift, Airship, HotAirBalloon, GasBalloon,
	...
	public String Class;
	public int AltCatClass;
	public CatClassID IdCatClass;

	public enum CatClassProp {pidCatClass, pidCategory, pidClass, pidAltCatClass, pidCatClassId};
	...
	public CategoryClass(SoapObject so)
	{
		super();
		Init(CatClassID.none);
		FromProperties(so);
	...
	{
		ArrayList<CategoryClass> lst = new ArrayList<CategoryClass>();
		for (CatClassID ccid : CatClassID.values())
			if (ccid != CatClassID.none)
			lst.add(new CategoryClass(ccid));
		return (CategoryClass[]) lst.toArray(new CategoryClass[0]);
	}
}
