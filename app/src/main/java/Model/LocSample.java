...
public class LocSample extends LatLong {
	...
	{
		ArrayList<LocSample> al = new ArrayList<LocSample>();
	...
	public static String FlightDataFromSamples(LocSample[] rgloc)
	{
		if (rgloc.length == 0)
			return "";
		...
		for (int i = 0; i < rgloc.length; i++)
			sb.append(String.format(Locale.US, "%.8f,%.8f,%d,%.1f,%.1f,%s,%d,%s\r\n", 
					...
					rgloc[i].Speed,
					rgloc[i].HError,
					getUTCFormatter().format(rgloc[i].TimeStamp),
					rgloc[i].TZOffset,
					rgloc[i].Comment));
		return sb.toString();
	}
	...
	public LocSample(Cursor c)
	{
		super();
		FromCursor(c);
	}
	
	public LocSample(Location l)
	{
		super();
	...
	public LocSample(double latitude, double longitude, int altitude, double speed, double error, String szDate)
	{
		super();
		Latitude = latitude;
		Longitude = longitude;
