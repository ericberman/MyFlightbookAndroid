...
public class FlightProperty extends SoapableObject implements KvmSerializable {
	...
	public static final String TABLENAME = "FlightProps";
	
	public enum FPProp {pidPropId, pidFlightID, pidPropTypeId, pidIntValue,
	...
	public FlightProperty(CustomPropertyType cpt)
	{
		super();
		this.m_cpt = cpt;
	...
	public static FlightProperty[] DistillList(FlightProperty[] rgfp)
	{
		...
		ArrayList<FlightProperty> al = new ArrayList<FlightProperty>();
	...
	public static FlightProperty[] FromDB(long id)
	{
		...
		if (id > 0)
		{
			...
			try
			{
				c = db.query(TABLENAME, null, "idFlight = ?", new String[] {String.format("%d", id)}, null, null, null);
	...
	public static void RewritePropertiesForFlight(long idFlight, FlightProperty[] rgfp)
	{
		if (idFlight > 0)
		{
			...
			try
			{
				...
				db.delete(TABLENAME, "idFlight = ?", new String[] {String.format("%d", idFlight)});
