...
public class DecimalEdit extends EditText implements OnLongClickListener {
	...
	public interface CrossFillDelegate { void CrossFillRequested(DecimalEdit sender);}
	...
	// Do we force the mode?  if false, HHMM will use HHMM if user wants it, otherwise will use decimal
	private boolean m_forceMode = false;
	...
	protected void InitFromAttributes(Context context, AttributeSet attrs)
	{
		...
		m_forceMode = arr.getBoolean(R.styleable.DecimalEdit_ForceMode, false);
	...
	public void setMode(EditMode value)
	{
		...
		case DECIMAL:
			...
			this.setHint(String.format("%.1f", 0.0));
	...
	{
		setText(d == 0 ? "" : String.format("%d", d));
	}
	
	public void setForceMode(boolean f)
	{
		m_forceMode = f;
	}
	...
	public void onTextChanged(CharSequence s, int start, int before, int after)
	{
		...
		if (EffectiveMode() == EditMode.HHMM)
		{
			...
			if (val == 0)
				...
				sz = String.format("%d:%02d", (int) (val / 100), val %  100);
			this.setText(sz);
