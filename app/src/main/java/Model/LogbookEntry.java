...
public class LogbookEntry extends SoapableObject implements KvmSerializable, LazyThumbnailLoader.ThumbnailedItem {
	...
	public enum FlightProp {pidFlightId, pidUser, pidFlightDate, pidCatClassOverride,
	...
	public enum SigStatus {None, Valid, Invalid}
	...
	public String szModelDisplay = "";
	public String szTailNumDisplay = "";
	public String szCatClassDisplay = "";
	public Boolean fHasDataStream = false;
	...
	public void AddImageForflight(MFBImageInfo mfbii)
	{
		...
		MFBImageInfo[] rgmfbNew = (this.rgFlightImages == null) ? new MFBImageInfo[1] : new MFBImageInfo[this.rgFlightImages.length + 1];
		int i = 0;
		for (MFBImageInfo m : this.rgFlightImages)
		{
			rgmfbNew[i] = m;
			i++;
		}
	...
	public Boolean IsDeletableFlight()
	{
		return !IsNewFlight();
	}
	...
	public Boolean FlightInProgress()
	{
	...
	public Object getProperty(int i)
	{
		...
		case pidProperties:
			...
			return new Vector<FlightProperty>(Arrays.asList(this.rgCustomProperties));
		case pidExistingImages:
			return new Vector<MFBImageInfo>(Arrays.asList(this.rgFlightImages));
		default:
			break;
		}
		return o;
	...
	public void DeletePendingFlight()
	{
		...
		try
		{
			String[] rgIdArg = {String.format("%d", this.idLocalDB)};
	...
	public boolean ToDB()
	{
		...
		try
		{
			if (this.idLocalDB <= 0)
				...
				db.update("Flights", cv, "_id = ?", new String[] {String.format("%d", this.idLocalDB)});
		}
		catch (Exception e)
		{
			fResult = false;
	...
	private void FromCursor(Cursor c)
	{
		...
		try
		{
			...
			fHold= Boolean.parseBoolean(c.getString(c.getColumnIndex("fHold")).toString());
			szComments= c.getString(c.getColumnIndex("szComments"));
			szRoute= c.getString(c.getColumnIndex("szRoute"));
			fPublic= Boolean.parseBoolean(c.getString(c.getColumnIndex("fPublic")).toString());
	...
	private void FromDB()
	{
		if (this.idLocalDB > 0)
		{
			...
			try
			{
				c = db.query("Flights", null, "_id = ?", new String[] {String.format("%d", this.idLocalDB)}, null, null, null);
	...
	public static LogbookEntry[] getFlightsWithIdFlight(int idFlight)
	{
		...
		try
		{
			c = db.query("Flights", null, "idFlight = ?", new String[] {String.format("%d", idFlight)}, null, null, null);
