...
public class ActVisitedAirports extends ExpandableListFragment implements MFBMain.Invalidatable {
	...
	private class SoapTask extends AsyncTask<Void, Void, MFBSoap>
	{
		...
		protected MFBSoap doInBackground(Void... params) {
			...
			VisitedAirport[] rgva = vas.VisitedAirportsForUser(AuthToken.m_szAuthToken);
    		m_Result = rgva;
			return vas;
		}
	...
	{		
		...
		HashMap<String, String> hmAllAirports = new HashMap<String, String>();
		hmAllAirports.put("sectionName", szKeyLast);
		headers.put(szKeyLast, hmAllAirports);
		
		hmAllAirports = new HashMap<String, String>();
		...
		ArrayList<HashMap<String, String>> alAllAirports = new ArrayList<HashMap<String, String>>();
		...
		for (int i = 0; i < visitedAirports.length; i++)
		{
			...
			alAirports = (childrenMaps.containsKey(szKey) ? childrenMaps.get(szKey) : new ArrayList<HashMap<String, String>>());
